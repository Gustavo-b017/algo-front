...
function HeapResults({ produto, marca }) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const url = `${BASE_URL}/heap?produto=${encodeURIComponent(produto)}&marca=${encodeURIComponent(marca)}&k=${k}&criterio=${criterio}&modo=${modo}`;
      ...
      if (!res.ok) {
        console.error("Erro ao buscar dados de heap:", text);
        throw new Error(text);
      }

      const json = text ? JSON.parse(text) : [];

      // compatível com resposta encapsulada
      const resultados = Array.isArray(json)
        ? json
      ...
      console.error('Erro ao buscar dados de heap:', err.message);
      setHeapResults([]);
    } finally {
      setLoading(false);
    }
  }, [produto, marca, k, criterio, modo, BASE_URL]);
  ...
  return (
    <div className="mt-4">
      ...
      <div className="d-flex gap-2 mb-3">
        <select className="form-select w-auto" value={modo} onChange={e => setModo(e.target.value)}>
          <option value="maior">K-maior</option>
          <option value="menor">K-menor</option>
        </select>

        <select className="form-select w-auto" value={criterio} onChange={e => setCriterio(e.target.value)}>
